/**
 * Patches ALLOCATION_EMBEDDED_HTML_B64 inside different._AIplatform_V4.0.html (decoded HTML).
 * Run: node scripts/patch_alloc_embedded_onboarding.mjs
 */
import fs from "fs";
...
const TARGET = path.join(ROOT, "different._AIplatform_V4.0.html");

const MARKER = "CRM onboarding wealth goals v1";

function patchDecoded(html) {
  let h = html;
  if (h.includes(MARKER)) {
    console.log("Already patched (marker present).");
    return h;
  }
  ...
  if (!h.includes(FUNDS_OLD)) throw new Error("FUNDS block not found");
  h = h.replace(FUNDS_OLD, FUNDS_NEW);

  const INSTR_OLD = "const INSTRUMENT_CATEGORIES=['Growth','Income','Stabilization','Protection'];";
  const INSTR_NEW =
    "var CRM_ONB_I18N={" +
    "en:{" +
    ...
    "};" +
    "var CRM_TM_LABELS={" +
    "en:{ac:{equities:'Equities',bonds:'Bonds',cash:'Cash & equivalents',property:'Real estate',commodities:'Commodities',multi:'Multi-asset'},reg:{switzerland:'Switzerland',usa:'United States',uk:'United Kingdom',global:'Global',asia:'Asia Pacific',emerging:'Emerging markets',europe:'Europe'},ind:{financials:'Financials',technology:'Information technology',real_estate:'Real estate',materials:'Materials',healthcare:'Healthcare',industrials:'Industrials',consumer:'Consumer',diversified:'Diversified'}}," +
    "'zh-CN':{ac:{equities:'股票',bonds:'债券',cash:'现金',property:'房地产',commodities:'商品',multi:'多资产'},reg:{switzerland:'瑞士',usa:'美国',uk:'英国',global:'全球',asia:'亚太',emerging:'新兴市场',europe:'欧洲'},ind:{financials:'金融',technology:'信息技术',real_estate:'房地产',materials:'原材料',healthcare:'医疗',industrials:'工业',consumer:'消费',diversified:'综合'}}," +
    "'zh-TW':{ac:{equities:'股票',bonds:'債券',cash:'現金',property:'房地產',commodities:'商品',multi:'多資產'},reg:{switzerland:'瑞士',usa:'美國',uk:'英國',global:'全球',asia:'亞太',emerging:'新興市場',europe:'歐洲'},ind:{financials:'金融',technology:'資訊科技',real_estate:'房地產',materials:'原物料',healthcare:'醫療',industrials:'工業',consumer:'消費',diversified:'綜合'}}," +
    "ja:{ac:{equities:'株式',bonds:'債券',cash:'現金同等',property:'不動産',commodities:'コモディティ',multi:'マルチアセット'},reg:{switzerland:'スイス',usa:'米国',uk:'英国',global:'グローバル',asia:'アジア',emerging:'新興国',europe:'欧州'},ind:{financials:'金融',technology:'IT',real_estate:'不動産',materials:'素材',healthcare:'ヘルスケア',industrials:'資本財',consumer:'消費',diversified:'分散'}}" +
    "};" +
    "function crmOnbTt(k){var p=CRM_ONB_I18N[currentLocale]||CRM_ONB_I18N.en;var f=CRM_ONB_I18N.en;return p[k]!==undefined?p[k]:f[k];}" +
    "function crmTmLabel(dim,key){var loc=currentLocale==='ja'?'ja':(currentLocale==='zh-CN'?'zh-CN':(currentLocale==='zh-TW'?'zh-TW':'en'));var L=CRM_TM_LABELS[loc]||CRM_TM_LABELS.en;var m=L[dim]||{};return m[key]||key;}" +
    "function crmRefreshOnbDom(){var w=document.getElementById('onb-wealth-wrap');if(!w)return;var g=(crmOnbTt('goals')||{});w.querySelectorAll('.onb-wealth-chip').forEach(function(chip){var id=chip.getAttribute('data-goal');var gg=g[id]||{};var l=chip.querySelector('.onb-lbl');var s=chip.querySelector('.onb-sub');if(l)l.textContent=gg.title||id;if(s)s.textContent=gg.sub||'';});var ak=document.getElementById('onb-asset-kicker');if(ak)ak.textContent=crmOnbTt('assetKicker');var at=document.getElementById('onb-asset-title');if(at)at.textContent=crmOnbTt('assetTitle');var ah=document.getElementById('onb-asset-hint');if(ah)ah.textContent=crmOnbTt('assetHint');var ag=(crmOnbTt('assets')||{});document.querySelectorAll('.onb-asset-chip').forEach(function(chip){var id=chip.getAttribute('data-asset');var a=ag[id]||{};var l=chip.querySelector('.onb-lbl');var s=chip.querySelector('.onb-sub');if(l)l.textContent=a.title||id;if(s)s.textContent=a.sub||'';});var wk=document.getElementById('onb-wealth-kicker');if(wk)wk.textContent=crmOnbTt('wealthKicker');var wt=document.getElementById('onb-wealth-title');if(wt)wt.textContent=crmOnbTt('wealthTitle');var wh=document.getElementById('onb-wealth-hint');if(wh)wh.textContent=crmOnbTt('wealthHint');var wr=document.getElementById('onb-wealth-warn');if(wr){wr.textContent=crmOnbTt('wealthRequired');wr.classList.remove('visible');}var tm=document.getElementById('onb-treemap-title');if(tm)tm.textContent=crmOnbTt('treemapTitle');var ts=document.getElementById('onb-treemap-sub');if(ts)ts.textContent=crmOnbTt('treemapSub');var h1=document.getElementById('onb-tm-h-ac');if(h1)h1.textContent=crmOnbTt('treemapAc');var h2=document.getElementById('onb-tm-h-reg');if(h2)h2.textContent=crmOnbTt('treemapReg');var h3=document.getElementById('onb-tm-h-ind');if(h3)h3.textContent=crmOnbTt('treemapInd');var df=document.getElementById('onb-tm-disclaimer');if(df)df.textContent=crmOnbTt('treemapDisclaimer');var pr=document.getElementById('parseCheckRisk');if(pr)pr.textContent=crmOnbTt('parseCheckPrefs');}" +
    "function crmAgg(rows,keyFn){var m={};rows.forEach(function(r){var k=keyFn(r);if(!k)return;m[k]=(m[k]||0)+r.w;});return Object.keys(m).map(function(k){return{name:k,value:Math.round(m[k]*10)/10};}).filter(function(x){return x.value>0;}).sort(function(a,b){return b.value-a.value;});}" +
    "function crmTreemapOption(seriesData){var isLight=document.documentElement.getAttribute('data-theme')==='light';var labelColor=isLight?'#0f172a':'#f8fafc';var border=isLight?'rgba(15,23,42,.1)':'rgba(255,255,255,.1)';var colors=['#5b6cfb','#22c55e','#fbbf24','#fb7185','#2dd4bf','#a855f7','#f97316','#38bdf8','#f43f5e','#84cc16','#eab308'];return{tooltip:{trigger:'item',formatter:function(p){return p.name+'<br/><b>'+p.value+'%</b>';}},animationDuration:420,animationDurationUpdate:560,series:[{type:'treemap',width:'100%',height:'100%',left:0,right:0,top:0,bottom:0,roam:false,nodeClick:false,breadcrumb:{show:false},label:{show:true,color:labelColor,fontWeight:600,fontSize:11,overflow:'truncate',ellipsis:'…'},upperLabel:{show:false},itemStyle:{borderColor:border,borderWidth:2,gapWidth:2,borderRadius:8},levels:[{itemStyle:{borderWidth:0,gapWidth:3}}],data:seriesData.map(function(d,i){return{name:d.name,value:d.value,itemStyle:{color:colors[i%colors.length]}};})}]};}" +
    ...
    "function renderAllocTreemaps(p){if(typeof echarts==='undefined')return;var pKey=currentPlanProfileKey||'moderate';var rows=FUNDS.filter(function(f){return f.cat===selectedInstrumentCategory;}).map(function(f){return{w:Number(f.pct[pKey]||0)||0,tmAc:f.tmAc,tmReg:f.tmReg,tmInd:f.tmInd};});var sum=rows.reduce(function(a,b){return a+b.w;},0);if(sum<=0)return;rows=rows.map(function(r){return{w:r.w/sum*100,tmAc:r.tmAc,tmReg:r.tmReg,tmInd:r.tmInd};});var a1=crmAgg(rows,function(r){return crmTmLabel('ac',r.tmAc);});var a2=crmAgg(rows,function(r){return crmTmLabel('reg',r.tmReg);});var a3=crmAgg(rows,function(r){return crmTmLabel('ind',r.tmInd);});crmDisposeTreemaps();function paint(id,idx,data){var el=document.getElementById(id);if(!el)return;var inst=echarts.init(el,null,{renderer:'svg'});__crmTmCharts[idx]=inst;inst.setOption(crmTreemapOption(data),true);}paint('onb-tm-ac',0,a1);paint('onb-tm-reg',1,a2);paint('onb-tm-ind',2,a3);}" +
    "function crmInitOnboardingUi(){var w=document.getElementById('onb-wealth-wrap');if(!w||w.getAttribute('data-crm-bound')==='1')return;w.setAttribute('data-crm-bound','1');w.querySelectorAll('.onb-wealth-chip').forEach(function(chip){chip.addEventListener('click',function(){this.classList.toggle('selected');var warn=document.getElementById('onb-wealth-warn');if(warn)warn.classList.remove('visible');});});document.querySelectorAll('.onb-asset-chip').forEach(function(chip){chip.addEventListener('click',function(){this.classList.toggle('selected');});});}" +
    "function crmPickDemoPrefs(variant){document.querySelectorAll('.onb-wealth-chip').forEach(function(c){c.classList.remove('selected');});var picks=variant==='client2'?['preserve','education','parents']:['retirement','preserve','grow','property'];picks.forEach(function(id){var el=document.querySelector('.onb-wealth-chip[data-goal=\"'+id+'\"]');if(el)el.classList.add('selected');});document.querySelectorAll('.onb-asset-chip').forEach(function(c){c.classList.remove('selected');});var ap=variant==='client2'?['cash','fixed']:['equity','fixed','re'];ap.forEach(function(id){var el=document.querySelector('.onb-asset-chip[data-asset=\"'+id+'\"]');if(el)el.classList.add('selected');});}" +
    INSTR_OLD;

  h = h.replace(INSTR_OLD, INSTR_NEW);

  const step0Needle =
    "        <!-- 已改为右侧录音解析操作流，左侧不再放 AI Conversation 解析按钮 -->\n        <div class=\"field-grid\">";
  const step0Insert =
    "        <!-- 已改为右侧录音解析操作流，左侧不再放 AI Conversation 解析按钮 -->" +
  ...
  if (!h.includes(step0Needle)) throw new Error("step0 needle not found");
  h = h.replace(step0Needle, step0Insert);

  const step1Needle = `      <div class="step-panel" id="step-1">
        <div class="step-head">
  ...
  const step1Insert = `      <div class="step-panel" id="step-1">
        <div class="step-head">
          <div class="step-head-text">
            <div class="step-title">Your Style</div>
            <div class="step-subtitle">Select the profile that best matches the client's investor DNA.</div>
  ...
  if (!h.includes(step1Needle)) throw new Error("step1 needle not found");
  h = h.replace(step1Needle, step1Insert);

  const tmNeedle =
    "          </div>\n          </div>\n          <!-- 业务说明：Growth 与 AI 摘要上下排列，加宽容器内自适应图表与文案 -->\n          <div class=\"plan-timeline-ai-stack\">";
  ...
  if (!h.includes(tmNeedle)) throw new Error("treemap needle not found");
  h = h.replace(tmNeedle, tmInsert);
  ...
  const riskPatchOld = "  if(checkedSet.includes('risk')){\n    selectedProfile=data.profile||'moderate';";
  const riskPatchNew =
    "  if(checkedSet.includes('risk')){\n    try{if(typeof crmPickDemoPrefs==='function')crmPickDemoPrefs(variant);}catch(e){}\n    selectedProfile=data.profile||'moderate';";
  if (h.includes(riskPatchOld)) h = h.replace(riskPatchOld, riskPatchNew);
  else console.warn("risk block patch skipped (pattern not found)");

  if (!h.includes(MARKER)) throw new Error("Marker missing after patch");
...
if (!m) throw new Error("B64 constant not found");
const prev = Buffer.from(m[1], "base64").toString("utf8");
const next = patchDecoded(prev);
if (next === prev) {
...
console.log("Updated embedded onboarding. Bytes:", Buffer.byteLength(next, "utf8"));
